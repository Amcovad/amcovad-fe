...
const NewFriendToast = ({ name, connectionId, successCallback }: any) => {
  const accept = async () => {
    try {
      await connectionService.accept(connectionId);
      // console.log(data);
      toast.success("Request accepted");
      successCallback(connectionId);
    } catch (error: any) {
    ...
    >
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center ">
          ...
          <div className="ml-3">
            <p className="text-sm font-medium font-Inter text-secondary-800">
              {/* Jumoke Philips wants to join your network */}
        ...
        <div>
          ...
          <Button onClick={accept} className="ml-1" size="xs">
            Accept
          </Button>
        </div>
      </div>
